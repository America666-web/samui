...
function smRenderSupportMessages(email) {
  ...
  box.innerHTML = thread.map(function (m) {
    ...
    const img = m.imageUrl ? '<a href="' + m.imageUrl + '" target="_blank" rel="noopener"><img src="' + m.imageUrl + '" style="max-width:180px;border-radius:8px;display:block;margin-top:4px;"></a>' : '';
    ...
    return `
      <div style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--border);">
        <div style="width:40px;height:40px;border-radius:50%;overflow:hidden;background:var(--bg-secondary);flex-shrink:0;">
          ${d.photo ? '<img src="'+d.photo+'" style="width:100%;height:100%;object-fit:cover;">' : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;color:var(--accent);">'+d.name.charAt(0).toUpperCase()+'</div>'}
        </div>
        <div style="flex:1;">
          <b>${smEsc(d.name)}</b>
